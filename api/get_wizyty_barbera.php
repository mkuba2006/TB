...
header("Access-Control-Allow-Methods: POST, OPTIONS"); // Oczekujemy POST z Reacta
...
// Odbieranie ID Barbera z ciała POST
$data = json_decode(file_get_contents("php://input"), true);
$id_barbera = $data['id_barbera'] ?? null;

if (empty($id_barbera)) {
    http_response_code(400);
    echo json_encode(["success" => false, "error" => "Brak ID barbera w żądaniu POST."]);
    exit();
}


// --- Złożone Zapytanie SQL (Dla JEDNEGO barbera) ---
$sql = "
    SELECT
        wu.id_wizyty,
        wu.data_wizyty,
    ...
    WHERE usl.id_barbera = :id_barbera /* FILTROWANIE PO ID BARBERA */
    ORDER BY wu.data_wizyty DESC
";

try {
    $stmt = $pdo->prepare($sql);
    $stmt->execute([':id_barbera' => $id_barbera]);
